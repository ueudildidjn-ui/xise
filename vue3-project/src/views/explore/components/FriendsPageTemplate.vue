...
import { getFriendsPosts } from '@/api/posts.js'
...
async function loadContent() {
    ...
    loading.value = true
    try {
        const result = await getFriendsPosts({
            page: 1,
            limit: pageSize,
        ...
        hasFriends.value = result.hasFriends
        recommendedUsers.value = result.recommendedUsers || []
        posts.value = result.posts || []
        hasMore.value = result.hasMore
        currentPage.value = 1
    } catch (error) {
        console.error('加载朋友内容失败:', error)
    } finally {
        loading.value = false
    }
}
...
async function loadMoreContent() {
    if (loadingMore.value || !hasMore.value || !hasFriends.value) return
    ...
    <div class="friends-page">
        ...
        <div v-if="!isLoggedIn" class="login-prompt">
            <div class="login-prompt-content">
                ...
                <p class="login-text">登录后查看朋友的动态</p>
        ...
        <div v-else>
            ...
            <!-- 无互相关注的朋友，显示推荐用户 -->
            <div v-else-if="!hasFriends && recommendedUsers.length > 0" class="recommend-container">
                <div class="recommend-header">
                    <h3 class="recommend-title">推荐关注</h3>
                    <p class="recommend-subtitle">互相关注即可成为朋友，查看朋友的最新动态</p>
                </div>
                <div class="user-grid">
            ...
            <!-- 无互相关注的朋友且无推荐用户 -->
            <div v-else-if="!hasFriends && recommendedUsers.length === 0" class="empty-state">
                <p class="empty-text">暂无互相关注的朋友</p>
            </div>
