...
/**
 * @swagger
 * /api/users/onboarding-config:
 *   get:
 *     summary: 获取初始设置页面配置
 *     description: 公开接口，不需要登录
 *     tags: [用户]
 *     responses:
 *       200:
 *         description: 获取成功
...
/**
 * @swagger
 * /api/users/onboarding-draft:
 *   get:
 *     summary: 获取引导页草稿
 *     description: 从 Redis 中获取引导页草稿
 *     tags: [用户]
 *     security:
 *       - bearerAuth: []
 *     responses:
 ...
 *               custom_fields:
 *                 type: object
 *                 description: 自定义字段（key最长20字符，value最长50字符）
 *     responses:
 *       200:
 *         description: 引导信息保存成功
 *       400:
 *         description: 参数无效
 *       500:
 *         description: 服务器内部错误
 ...
 *               privacy_mbti:
 *                 type: boolean
 *                 description: 是否公开MBTI
 *               privacy_custom_fields:
 *                 type: object
 *                 description: 自定义字段隐私设置
 *     responses:
 *       200:
 *         description: 隐私设置更新成功
 *       400:
 *         description: 没有需要更新的设置
 *       500:
 *         description: 服务器内部错误
 */
// 更新隐私设置
 ...
 *   delete:
 *     summary: 删除账号
 *     description: 只能删除自己的账号，同时删除所有关联数据
 *     tags: [用户]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *           type: string
 *           default: created_at
 *           enum: [created_at, view_count, like_count, collect_count, comment_count]
 *         description: 排序字段
 *       - in: query
 *         name: visibility
 *         schema:
 ...
 *       400:
 *         description: 缺少必填字段或无效的认证类型
 *       409:
 *         description: 已有待审核或已通过的认证申请
 *       500:
 *         description: 服务器内部错误
 */
// 提交认证申请
...
/**
 * @swagger
 * /api/users/verification/revoke:
 *   delete:
 *     summary: 撤回认证申请
 *     description: 撤回认证申请，如果已通过则重置认证状态
 *     tags: [用户]
 *     security:
 *       - bearerAuth: []
 *     responses:
